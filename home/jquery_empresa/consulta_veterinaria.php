...
switch($opcion){
    case 1:
        ...
        if ($idproducto != '' || $idproducto != '0') {
         ...
         $query_verificador_estado = mysqli_query($conection, "SELECT * FROM  consulta_veterinaria  WHERE  consulta_veterinaria.id_emisor  = $iduser AND consulta_veterinaria.IDROLPUNTOVENTA='ADMIN' ORDER BY id DESC LIMIT 1");
        ...
        $consulta = "INSERT INTO consulta_veterinaria (id_emisor,nombre_producto, descripcion_producto, valor_unidad,cantidad_producto,tipo_ambiente,codigos_impuestos,detalle_extra,precio_neto,iva_producto,precio_p_incluido_iva,id_producto,descuento,iva_frontend,subtotal_frontend,imagen,secuencial,consulta)
        VALUES('$iduser','$nombre_producto', '$descripcion_producto', '$valor_unidad','$cantidad_producto','$tipo_ambiente','$codigos_impuestos','$detalle_extra','$precio_neto','$iva_cantidad','$precio_p_incluido_iva','$idproducto','$cantidad_descuento','$iva_frontend','$subtotal_frontend','$foto','$secuencial','$consulta') ";
        $resultado = $conexion->prepare($consulta);
        $resultado->execute();

        $consulta = "SELECT * FROM consulta_veterinaria WHERE  estado_pedido ='INICIADO' AND id_emisor = '$iduser' AND consulta_veterinaria.consulta ='$consulta'  ORDER BY id  DESC LIMIT 1";
        $resultado = $conexion->prepare($consulta);
        $resultado->execute();
    ...
    case 3:
        ...
        break;
    case 4:
        $consulta = "SELECT * FROM consulta_veterinaria WHERE id_emisor ='$iduser' AND consulta_veterinaria.estado_pedido ='INICIADO' AND consulta_veterinaria.IDROLPUNTOVENTA='ADMIN' AND consulta_veterinaria.consulta ='$consulta' ORDER BY id DESC" ;
        $resultado = $conexion->prepare($consulta);
        $resultado->execute();
        $data=$resultado->fetchAll(PDO::FETCH_ASSOC);
        break;
    case 77:
       ...
       $data=$resultado->fetchAll(PDO::FETCH_ASSOC);
        break;
    case 8:
      $consulta = "DELETE FROM consulta_veterinaria WHERE id_emisor='$iduser' AND consulta_veterinaria.estado_pedido ='INICIADO' AND consulta_veterinaria.IDROLPUNTOVENTA='ADMIN'  ";
      $resultado = $conexion->prepare($consulta);
      $resultado->execute();
      $data=$resultado->fetchAll(PDO::FETCH_ASSOC);
      $consulta = "SELECT * FROM consulta_veterinaria WHERE id_emisor='$iduser'";
      $resultado = $conexion->prepare($consulta);
      $resultado->execute();
      $data=$resultado->fetchAll(PDO::FETCH_ASSOC);
      break;
      ...
      $consulta = "UPDATE consulta_veterinaria SET estado_pedido='ENVIADO',qr_imagen = '$qr_img',qr_contenido='$contenido',codigo_mesa = '$codigo_mesa'  WHERE id_emisor='$iduser' AND secuencial = '$pedido_generado' AND consulta_veterinaria.IDROLPUNTOVENTA='ADMIN' ";
      $resultado = $conexion->prepare($consulta);
      $resultado->execute();

      $consulta = "SELECT * FROM consulta_veterinaria WHERE id_emisor='$iduser' AND estado_pedido ='INICIADO' ORDER BY id DESC ";
      $resultado = $conexion->prepare($consulta);
      $resultado->execute();
      $data=$resultado->fetchAll(PDO::FETCH_ASSOC);
