...
<html lang="es">
    <head>
        ...
        <meta property="og:url" content="https://www.guibis.com/suscripcion?codigo=<?php echo $producto ?>" />
                          ...
                          <div class="card">
                              <div class="card-block">
                                               ...
                                               <a href="code_user?code=<?php echo $id_e_empresa?>">Visita toda la tienda</a>
                                                <div class="calificacion_resultado_at" id="calificacion_resultado_at"></div>
                                                 ...
                                                 <div class="categorizadas_at">
                                                   ...
                                                   <p>Meses de Suscripción:<?php echo $data_producto['meses_suscripcion'] ?> </p>
                                                   <p>Cantidad:<?php echo $data_producto['cantidad'] ?> </p>
                                ...
                                <?php endif; ?>
                                   ...
                                   <?php if ($id_empresa == $iduser): ?>
                                     <div class="container guia_general mt-4">
                                            <div class="titulo_guia mb-3">
                                              <h3 class="text-secondary">Guía para Usar el API REST en Servicio de Suscripción</h3>
                                            </div>
                                            <div class="mb-3">
                                              <p>Para el uso de esta tecnología el usuario que está instalando la verificación del pago de un servicio por suscripción deberá tener los siguientes requisitos.</p>
                                            </div>
                                            <div class="lista_requisitos mb-3">
                                              <ol class="list-group list-group-numbered">
                                                <li class="list-group-item">Certificado SSL.</li>
                                                <li class="list-group-item">Tener Activado la Cuenta Digital Bancaria en nuestro sitio.</li>
                                                <li class="list-group-item">Tener al menos un servicio de suscripción.</li>
                                              </ol>
                                            </div>
                                            <div class="j_parrafo mb-3">
                                              <p>El código que se encuentra en nuestro sitio deberá de ser integrado en la pagina web del usuario que creo el servicio de suscripción, para ello existen dos variables, la variable del email del usuario de quien contrata el servicio y el ID del servicio de suscripción, con estas dos variables mediante una API REST en PHP.</p>
                                            </div>
                                            <p class="fw-bold">Código Fuente:</p>
                                            <div class="text_codigo mb-3">
                                              <textarea class="form-control" id="codigo_api_suscripcion" name="name" rows="8" readonly>
                                                $datos = json_decode(file_get_contents('https://guibis.com/api/suscripciones/check?producto=<?php echo $producto ?>&email=guibis-ecuador@guibis.com'),true);
                                                  print_r($datos);

                                                  <?php
                                                  $datos = json_decode(file_get_contents('https://guibis.com/api/suscripciones/check?producto='.$producto.'&email=guibis-ecuador@guibis.com'),true);
                                                  print_r($datos);
                                                   ?>
                                              </textarea>
                                            </div>
                                            <div class="mb-3">
                                              <p>En la web del usuario que crea el evento de recaudación se tiene que tener un inicio de sesión hecho en cualquier lenguaje de programación, consumir una API REST y la respuesta viene un arreglo que contiene la siguiente información.</p>
                                            </div>
                                            <div class="mb-3">
                                              <h4 class="text-primary">Email del usuario</h4>
                                              <p>En el primer campo es el email del usuario que compro el paquete de suscripción quien esta registrado en https://guibis.com.</p>
                                              <h4 class="text-primary">Nombres</h4>
                                              <p>Vienen los nombres del usuario que compro la suscripción mediante nuestra plataforma, los nombres no se pueden cambiar por seguridad.</p>
                                              <h4 class="text-primary">ID Venta</h4>
                                              <p>El id de la venta también se puede consultar en mis ventas, y ver con más detalles el proceso y estado de la cuenta.</p>
                                              <h4 class="text-primary">Fecha límite de suscripción</h4>
                                              <p>Al momento de comprar se elige el tiempo de suscripción y mediante la consulta en el ordenador la fecha actual se puede establecer la caducidad de la suscripción.</p>
                                              <h4 class="text-primary">Estado de la venta</h4>
                                              <p>Existen cuatro estados de la venta (Iniciada, Cancelada, Reportada y Finalizada), mediante la cual el propietario del servicio de suscripción puede crear mensajes mediante el estado de la venta.</p>
                                              <h4 class="text-primary">Estado de Reporte</h4>
                                              <p>Si la venta ha sido reportada entra en un proceso con nuestro departamento legal, si no existe ningún reporte el servicio se puede dar sin ningún problema.</p>
                                            </div>
                                          </div>
                                   ...
                                   <?php endif; ?>
                                     ...
                                     <?php if (!empty($data_producto['longitud_user_producto'])): ?>
                                          ...
                                          <style media="screen">
                                            .contenedor_existencia_video{
                                              text-align: center;
                                            }
                                            .contenedor_existencia_video #my-video{
                                              margin: 0 auto;
                                            }
                                          </style>
        ...
        <div class="modal-content">
          ...
          <div class="modal-body">
            <form id="comprar_articulo_inicial" method="post" onsubmit="event.preventDefault(); sendData_comprar_articulo();">
              ...
              <div class="modal-footer">
                <input type="hidden" name="action" value="comprar_evento_suscripcion">
...
<script src="https://cdnjs.cloudflare.com/ajax/libs/Chart.js/3.9.1/chart.esm.min.js"></script>
      ...
      var url = 'https://guibis.com/suscripcion?codigo=<?php echo $producto ?>&name=<?php echo $data_producto['nombre'] ?>'; // Reemplaza con la URL dinámica si es necesario
