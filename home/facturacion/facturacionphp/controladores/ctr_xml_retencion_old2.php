...
class xml{
  ...
  include "../../../../coneccion.php";
    ...
    //INFORMACION DE LA CONFIGURACION
		...
		//$xml_tireo    = $xml->createElement('tipoRegi','01');
		...
		foreach ($contador_tipo_pagos as $Item) {
				$xml_pago[1] = $xml->createElement('Pago');
				$xml_fpgo[1] = $xml->createElement('formapago', '01'); // Reemplaza '01' con el valor correspondiente si es necesario
				$xml_ttal[1] = $xml->createElement('total', '500'); // Reemplaza '500' con el valor correspondiente si es necesario
				$base_pago++;
		}
	...
	//	$xml_rede->appendChild($xml_nadr);
		...
		$base_pago_2 = 1;
		foreach ($contador_tipo_pagos as $Item) {
				$xml_pags->appendChild($xml_pago[1]);
				$xml_pago[1]->appendChild($xml_fpgo[1]);
				$xml_pago[1]->appendChild($xml_ttal[1]);
				$base_pago_2++;
		}
