 ...
 $query_consultor = mysqli_query($conection,"SELECT * FROM comprobantes
   ...
   if ($result_consultor) {
    ...
    if (empty($result_documentos['numero_identidad'])) {
     ...
     $query_secuencial = mysqli_query($conection, "SELECT * FROM  tikets  WHERE  tikets.id_emisor  = $iduser ORDER BY fecha DESC LIMIT 1");
     ...
     $xmla=new autorizar_ticket();
     $xmla->autorizar_xml_tiket($codigo_factura);
