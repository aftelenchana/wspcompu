<?php
set_time_limit(300);
require_once './ctr_funciones_guia_remision.php';
require_once './ctr_pdf_guia_remision.php';
require_once '../lib/nusoap.php';
//if (isset($_POST['submit'])) {
class autorizar {
	public function autorizar_xml($clave_acceso_factura,$direccion_partida,$razon_social_transportista,$tipoIdentificacionTransportista,$fecha_inicio_transporte,$fecha_final_transporte,$placa_transportista,$ruc_transportista,$direccion_llegada,$motivo_traslado) {
	 ...
	 $ambito_area          =  $result_configuracion['ambito'];
			...
			$firma_electronica = $resulemisor['firma_electronica'];
			 	...
			 	//$func = new fac_ele();
			 	//$pdf = new pdf();
			 	//$clave_acc_guardar = '1602202301171685757600120010010000003401234567814';
			 	//$pdf->pdfFactura($clave_acceso_factura,$direccion_partida,$razon_social_transportista,$tipoIdentificacionTransportista,$fecha_inicio_transporte,$fecha_final_transporte,$placa_transportista,$ruc_transportista,$direccion_llegada,$motivo_traslado,$clave_acc_guardar);
			 	//$func->correos($clave_acceso_factura,$direccion_partida,$razon_social_transportista,$tipoIdentificacionTransportista,$fecha_inicio_transporte,$fecha_final_transporte,$placa_transportista,$ruc_transportista,$direccion_llegada,$motivo_traslado,$clave_acc_guardar);
			 	//$func->compr_final($clave_acceso_factura,$direccion_partida,$razon_social_transportista,$tipoIdentificacionTransportista,$fecha_inicio_transporte,$fecha_final_transporte,$placa_transportista,$ruc_transportista,$direccion_llegada,$motivo_traslado,$clave_acc_guardar);
	 ...
	 $clave = ''.$codigo_sri.'';
		...
		if (openssl_pkcs12_read($almacen_cert, $info_cert, $clave)) {
			...
			if ($ambito_area == 'prueba') {
				$ruta_no_firmados = 'C:\\xampp\\htdocs\\home\\facturacion\\facturacionphp\\comprobantes\\guia-remision\\no_firmados\\guiaremision'.$iduser.'.xml';
				$ruta_si_firmados = 'C:\\xampp\\htdocs\\home\\facturacion\\facturacionphp\\comprobantes\\guia-remision\\si_firmados\\';
				$ruta_autorizados = 'C:\\xampp\\htdocs\\home\\facturacion\\facturacionphp\\comprobantes\\guia-remision\\autorizados\\';
				$pathPdf = 'C:\\xampp\\htdocs\\home\\facturacion\\facturacionphp\\comprobantes\\guia-remision\\pdf\\';
			}else {
				$ruta_no_firmados = '../comprobantes/guia-remision/no_firmados/guiaremision'.$iduser.'.xml';
				$ruta_si_firmados = '../comprobantes/guia-remision/si_firmados/';
				$ruta_autorizados = '../comprobantes/guia-remision/autorizados/';
				$pathPdf = '../comprobantes/guia-remision/pdf/';
			}
				...
				case 'FIRMADO':
					...
					case 'RECIBIDA':
						...
						case 'AUTORIZADO':
							...
							$arrayName = array('noticia'=>'guia_emision_creada','clave_acceso' =>$clave_acc_guardar );
							echo json_encode($arrayName,JSON_UNESCAPED_UNICODE);
...
//MANDAR UN MENSAJE IMPORTANTE
						...
						break;
					case 'DEVUELTA':
					var_dump($response["RespuestaRecepcionComprobante"]["comprobantes"]["comprobante"]["mensajes"]["mensaje"]);
