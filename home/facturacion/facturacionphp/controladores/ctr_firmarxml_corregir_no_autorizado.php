<?php
set_time_limit(300);
require_once './ctr_funciones_correccion_autorizacion.php';
require_once '../lib/nusoap.php';
//require_once 'ruth_esmeralda_sanchez_herrera.p12';
//if (isset($_POST['submit'])) {
class autorizar {
	public function autorizar_xml($iduser,$clave_acceso_factura) {
		include "../../../../coneccion.php";
			include "ctr_pdf_new_correccion.php";
	 $iduser= $_SESSION['id'];
		...
		if (openssl_pkcs12_read($almacen_cert, $info_cert, $clave)) {
			...
			$codigo_factura = $iduser.md5(date('d-m-Y H:m:s')).$clave_acceso_factura;
								...
								case 'AUTORIZADO':
									...
									if ($contenido_colateral_rt == 'genear_factura_y_nota_credito') {
											...
											$xmla->generate_pdf($clave_acc_guardar);
												...
												$crecion = crecion($clave_acc_guardar,$fechaAutorizacion,$clave_acceso_factura);
									}

									$func->correos($clave_acc_guardar,$numDocModificado,$clave_acceso_factura);
									$func->compr_final($clave_acc_guardar,$numDocModificado,$fechaAutorizacion,$clave_acceso_factura);
