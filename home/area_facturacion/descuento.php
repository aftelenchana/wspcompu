...
if ($_POST['action'] == 'editar_descuento_producto') {
  ...
  $id_producto = $data_comprobante['id_producto'];
   ...
   $query_producto = mysqli_query($conection,"SELECT producto_venta.idproducto,producto_venta.precio,producto_venta.precio_costo,producto_venta.nombre,producto_venta.valor_unidad_final_con_impuestps,
    ...
    //codigo para sacar la informacion del orecio real de cada producto
  ...
  if ($query_edit) {
   ...
   'iva_frontend' =>$iva_frontend,'subtotal_frontend' =>$subtotal_frontend,'cantidad' =>$cantidad_producto,'idItem' =>$idItem,'porcentaje_descuento' =>$porcentaje_descuento,'cantidad_descuento' =>$cantidad_descuento,'codigo_item' =>$idItem);
