$(document).ready(function() {
    ...
    var tabla_email_envio = $('#tabla_email_envio').DataTable({
        "ajax": {
            "url": "jquery_administrativo/cuentas_email.php",
            "type": "POST",
            "data": {
    ...
    function sendData_agregar_email(){
        ...
        '</div>');
        var parametros = new FormData($('#add_email')[0]);
        $.ajax({
            data: parametros,
            url: 'jquery_administrativo/cuentas_email.php',
            type: 'POST',
            ...
            success: function(response){
                ...
                if (response =='error') {
                    ...
                    if (info.noticia == 'insert_correct') {
                                    ...
                                    '</div>');
                                    tabla_email_envio.ajax.reload(); // Recargar los datos en la tabla
                                }
    ...
    $('#tabla_email_envio').on('click', '.eliminar_cuenta_email', function(){
    ...
    $('#add_email').on('submit', function(e) {
...
(function() {
  $(function() {
    $('#boton_agregar_email').on('click', function() {
      $('#modal_agregar_email').modal();
      $("#nombre_almacen").val('');
      $("#responsable").val('');
