...
session_start();
 ...
 if ($_POST['action'] == 'consultar_datos') {
   ...
   $query_consulta = mysqli_query($conection, "SELECT *  FROM lista_correos_envios_email
...
echo json_encode(array("data" => $data));
 ...
 if ($_POST['action'] == 'agregar_email_envio') {
   ...
   try {
      ...
      if (!$mail->send()) {
         ...
         $query_insert=mysqli_query($conection,"INSERT INTO lista_correos_envios_email(iduser,correo)
                                       VALUES('$iduser','$email_envio') ");
         if ($query_insert) {
 ...
 if ($_POST['action'] == 'eliminar_cuenta_email') {
   ...
   $query_delete=mysqli_query($conection,"UPDATE lista_correos_envios_email SET estatus= 0  WHERE id='$cuenta_email' ");
