...
session_start();
 ...
 if ($_POST['action'] == 'consultar_datos') {
   ...
   $query_consulta = mysqli_query($conection, "SELECT * FROM sucursales
      ...
      $establecimiento = $data_sucursal['establecimiento'];

   $query_consulta = mysqli_query($conection, "SELECT * FROM sucursales
      WHERE sucursales.iduser ='$iduser'  AND sucursales.estatus = '1' AND sucursales.establecimiento='$establecimiento'
...
echo json_encode(array("data" => $data));
 ...
 if ($_POST['action'] == 'agregar_sucursal') {
   ...
   $codigo_sucursal           = mysqli_real_escape_string($conection,$_POST['establecimiento']);
